...
UNARY_SOURCE = ROOT / "TileOPs/src/tileops/kernels/elementwise_unary.py"
MIXED_SOURCE = ROOT / "TileOPs/src/tileops/kernels/elementwise_mixed.py"
...
def _single(op: str, dtype_name: str, variant: str) -> dict:
    ...
    try:
        if op in UNARY_OPS:
            from tileops.kernels.families.elementwise_unary_math import (  # noqa: PLC0415
                build_ceil,
                build_erf,
                build_floor,
                build_round,
            ...
            from tileops.kernels.families.elementwise import (  # noqa: PLC0415
                build_masked_fill,
...
def _clear_child_cache(op: str, cache_dir: Path) -> None:
    ...
    module_name = "tileops.kernels.elementwise_mixed" if op.startswith("MaskedFill") else "tileops.kernels.elementwise_unary"
...
def run(op: str, out_dir: Path, data_dir: Path) -> dict:
    ...
    result = {
        ...
        "correctness": {"passed": passed, "cmd": f"python -u TileOPs/tests/npu/t087_coverage.py --op {op} --out tileops-ascend-harness/coverage", "per_case": cases},
