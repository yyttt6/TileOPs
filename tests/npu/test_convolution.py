...
from tileops.kernels.families.convolution import _padding_pair, _pair, build_conv2d
...
def test_cube_and_direct_dispatch(monkeypatch):
    marker = object()
    monkeypatch.setattr(
        "tileops.kernels.convolution._compile_cube_conv2d",
        lambda *args: marker,
    )
    monkeypatch.setattr(
        "tileops.kernels.convolution._compile_direct_conv2d",
        lambda *args: marker,
    )
