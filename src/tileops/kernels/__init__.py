"""The kernels: one AI Core implementation per op, written in TileLang-Ascend.

`tileops.kernels.families` holds one module per op family, each naming its ops with
`_registry.register`; the modules beside this one hold the TileLang kernels those
builders call. A family module owns its ops exclusively, which is what lets several
executors work in parallel without touching a shared file (docs/DECISIONS.md D007).

Importing this package performs the whole registration, through the same
`tileops.backend` seam an out-of-tree backend would use -- these kernels are not
privileged for being in this repository. `pyproject.toml` declares this module in the
``tileops.backends`` entry-point group, so `tileops.backend` imports it while
constructing its first op and the calls at the bottom fill its registry.
...
to some other implementation. Registering a detector before all 178 ops are covered
would therefore make the whole target unusable rather than partially usable.
...
detector and have plain ``GemmFwdOp()`` pick this target from the input device --
