...
through a plain ``import tileops_ascend``.

Why this test exists (2026-08-26): ``families/__init__.py`` imported only
``elementwise``. A plain ``import tileops_ascend`` -- which is what the
...
import tileops_ascend  # noqa: F401  -- the plain import under test
from tileops_ascend._registry import REGISTERED

_FAMILIES_DIR = os.path.join(os.path.dirname(tileops_ascend.__file__), "families")
...
def test_plain_import_reaches_every_family_module():
    ...
    declared = set(tileops_ascend.families.__all__)
...
def test_plain_import_registers_every_op():
    ...
    for name in _family_module_names():
        importlib.import_module(f"tileops_ascend.families.{name}")
    after_manual_import = set(REGISTERED)
    pending_ops = set()
    for name in PENDING_PM_INTEGRATION:
        mod = importlib.import_module(f"tileops_ascend.families.{name}")
        pending_ops |= {
    ...
    assert not hidden, (
        ...
        f"and are therefore unreachable through a plain `import tileops_ascend`. "
